...
from .bedrock_client import call_bedrock_for_solicitud
...
def ia_chat(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        # Llamar a Claude (Bedrock) para extraer metadata
        try:
            payload = call_bedrock_for_solicitud(texto, image_bytes=image_bytes)
            contexto["payload"] = payload
        except Exception as e:
            messages.error(
                request,
                f"Error al procesar con IA: {e}",
            )
            return render(request, "ia/chat.html", contexto)
